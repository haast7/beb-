...
interface LeadFormData {
  ...
  position: string;
  segment: string;
  customSegment: string; // Novo campo para segmento personalizado
...
export function LeadFormModal({ isOpen, onClose, source, initialMessage }: LeadFormModalProps) {
  ...
  const validateStep2 = () => {
    ...
    if (!formData.segment) newErrors.segment = 'Segmento é obrigatório';
    
    // Se selecionou "Outro", o campo customSegment é obrigatório
    if (formData.segment === 'Outro' && !formData.customSegment.trim()) {
      newErrors.customSegment = 'Por favor, especifique o segmento';
    }
    
    if (!formData.monthlyRevenue) newErrors.monthlyRevenue = 'Faturamento é obrigatório';
                ...
                >
                  <div className="text-center mb-8">
                    ...
                    <p className="text-gray-600">
                      Preencha seus dados para começarmos nossa conversa
                    </p>
                  </div>
              ...
              {currentStep === 2 && (
                ...
                >
                  <div className="text-center mb-8">
                    ...
                    <p className="text-gray-600">
                      Isso nos ajuda a personalizar nossa abordagem
                    </p>
                  </div>
                  ...
                  {/* Campo personalizado que aparece quando "Outro" é selecionado */}
                  {formData.segment === 'Outro' && (
                    ...
                    >
                      ...
                      <input
                        ...
                        className={`w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-haast-primary focus:border-transparent transition-all ${
                          errors.customSegment ? 'border-red-500' : 'border-gray-300'
                        }`}
                        placeholder="Ex: Clínica de Fisioterapia, Laboratório de Genética, etc."
                      />
                      {errors.customSegment && (
                        <p className="text-red-500 text-sm mt-1">{errors.customSegment}</p>
                      )}
                    </motion.div>
                  )}
